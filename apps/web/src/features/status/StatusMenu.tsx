  ...
  if (isIosDevice(userAgent)) {
    if (!isStandalone) {
      return {
        steps: [
          ...
          "In iPhone or iPad Settings, allow notifications for Codex Remote."
        ],
    ...
    return {
      steps: [
        ...
        "Find Codex Remote and turn on Allow Notifications.",
...
export function StatusMenu({ isMobileViewport }: Props) {
  ...
  const homeScreenRow = useMemo(() => {
    ...
    if (deferredPrompt) {
      return {
        title: "Home screen",
        detail: "Add Codex Remote to your home screen.",
